use anyhow::Result;
use stage_forward_lab::{LoadedRuntimeBundle, StageResidencyAdapter};
...
fn main() -> Result<()> {
    ...
    println!("stages             : {}", bundle.stages.len());

    for stage in &bundle.stages {
        let adapter = StageResidencyAdapter::load(&bundle_root, &gguf_path, stage.stage_index)?;
        let out_dir = bundle_root.join(adapter.default_required_pack_dir_name());
