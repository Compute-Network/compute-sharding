...
#define LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY 1
    ...
    // Stage-split decode helpers.
    // - llama_decode_head() processes token ingress through end_layer and returns hidden-state embeddings.
    // - llama_decode_middle() injects hidden state through embd, while token ids may still be present for
    //   model-specific auxiliary paths (for example Gemma 4 per-layer token inputs).
    // - llama_decode_tail() injects hidden state through embd starting at start_layer and leaves logits in
    //   the normal llama.cpp output buffers for sampling.
    LLAMA_API int32_t llama_decode_head(
            struct llama_context * ctx,
              struct llama_batch   batch,
                       int32_t      end_layer);

    LLAMA_API int32_t llama_decode_middle(
            struct llama_context * ctx,
              struct llama_batch   batch,
                       int32_t      start_layer,
                       int32_t      end_layer);

    LLAMA_API int32_t llama_decode_tail(
            struct llama_context * ctx,
              struct llama_batch   batch,
                       int32_t      start_layer);
