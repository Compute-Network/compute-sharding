...
int llama_context::decode(const llama_batch & batch_inp) {
    GGML_ASSERT(batch_inp.token != nullptr || batch_inp.embd != nullptr); // NOLINT
...
int llama_context::decode_stage(const llama_batch & batch_inp, int32_t start_layer, int32_t end_layer, bool output_embeddings) {
    const int32_t n_layer = llama_model_n_layer(&model);

    if (start_layer < 0 || start_layer >= n_layer) {
        LLAMA_LOG_ERROR("%s: invalid start_layer = %d for n_layer = %d\n", __func__, start_layer, n_layer);
        return -1;
    }
    ...
    if (end_layer < start_layer || end_layer >= n_layer) {
        LLAMA_LOG_ERROR("%s: invalid end_layer = %d for start_layer = %d and n_layer = %d\n",
                __func__, end_layer, start_layer, n_layer);
        return -1;
    }

    const bool saved_embeddings = cparams.embeddings;
    const bool saved_split_active = split_stage_active;
    const int32_t saved_split_start = split_stage_start_layer;
    const int32_t saved_split_end = split_stage_end_layer;

    cparams.embeddings = output_embeddings;
    split_stage_active = true;
    split_stage_start_layer = start_layer;
    split_stage_end_layer = end_layer;

    const int ret = decode(batch_inp);

    cparams.embeddings = saved_embeddings;
    split_stage_active = saved_split_active;
    split_stage_start_layer = saved_split_start;
    split_stage_end_layer = saved_split_end;
...
int32_t llama_decode_head(
        llama_context * ctx,
          llama_batch   batch,
               int32_t  end_layer) {
    const int ret = ctx->decode_stage(batch, 0, end_layer, true);
    if (ret != 0 && ret != 1) {
        LLAMA_LOG_ERROR("%s: failed to decode head, ret = %d\n", __func__, ret);
    }

    return ret;
}

int32_t llama_decode_middle(
        llama_context * ctx,
          llama_batch   batch,
               int32_t  start_layer,
               int32_t  end_layer) {
    const int ret = ctx->decode_stage(batch, start_layer, end_layer, true);
    if (ret != 0 && ret != 1) {
        LLAMA_LOG_ERROR("%s: failed to decode middle, ret = %d\n", __func__, ret);
    }

    return ret;
}

int32_t llama_decode_tail(
        llama_context * ctx,
          llama_batch   batch,
               int32_t  start_layer) {
    const int ret = ctx->decode_stage(batch, start_layer, -1, false);
    if (ret != 0 && ret != 1) {
        LLAMA_LOG_ERROR("%s: failed to decode tail, ret = %d\n", __func__, ret);
    }
