...
void llm_graph_input_embd::set_input(const llama_ubatch * ubatch) {
    const char * LLAMA_STAGE_GRAPH_INPUT_DEBUG = getenv("LLAMA_STAGE_GRAPH_INPUT_DEBUG");
    if (LLAMA_STAGE_GRAPH_INPUT_DEBUG) {
        fprintf(stderr,
                "[llama-graph] set_input embd this=%p tokens=%p embd=%p ubatch_token=%p ubatch_embd=%p n_embd=%lld\n",
                (void *) this, (void *) tokens, (void *) embd, (void *) ubatch->token, (void *) ubatch->embd, (long long) n_embd);
    }
    if (ubatch->token && tokens && tokens->buffer) {
        const int64_t n_tokens = ubatch->n_tokens;

        ggml_backend_tensor_set(tokens, ubatch->token, 0, n_tokens*ggml_element_size(tokens));
    }

    if (ubatch->embd && embd && embd->buffer) {
...
ggml_tensor * llm_graph_context::build_inp_embd(ggml_tensor * tok_embd) const {
    return build_inp_embd_select(tok_embd, prefer_embd_input);
}

ggml_tensor * llm_graph_context::build_inp_embd_select(ggml_tensor * tok_embd, bool use_embd_input) const {
    ...
    if (LLAMA_STAGE_GRAPH_INPUT_DEBUG) {
        fprintf(stderr,
                "[llama-graph] build_inp_embd_select this=%p use_embd_input=%d inp=%p tokens=%p embd=%p n_tokens=%lld n_embd_inp=%lld\n",
                (void *) this, use_embd_input ? 1 : 0, (void *) inp.get(), (void *) inp->tokens, (void *) inp->embd,
                (long long) ubatch.n_tokens, (long long) n_embd_inp);
    }
    ...
    ggml_tensor * cur = ggml_build_forward_select(gf, inps.data(), inps.size(), use_embd_input ? 1 : 0);
