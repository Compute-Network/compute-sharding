...
llm_build_gemma4_iswa::llm_build_gemma4_iswa(const llama_model & model, const llm_graph_params & params) :
    ...
    ggml_tensor * cur;
    ggml_tensor * inpL;
    ggml_tensor * inp_prompt;

    inp_prompt = ubatch.token ? build_inp_embd_select(model.tok_embd, false) : build_inp_embd(model.tok_embd);
    ...
    if (model.per_layer_tok_embd) {
        ...
        inp_per_layer = project_per_layer_inputs(inp_prompt, inp_per_layer);
    }
...
ggml_tensor * llm_build_gemma4_iswa::build_inp_per_layer() {
    ...
    if (ubatch.token) {
        ...
        // Downstream split stages can carry both token ids and injected hidden-state embeddings in the same ubatch.
        // llm_graph_input_embd::set_input() updates both sides when ubatch.embd is present, so provide a matching
        // dummy embedding input here even though this path only consumes tokens for per-layer prompt auxiliaries.
